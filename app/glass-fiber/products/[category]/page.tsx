...
export async function generateStaticParams() {
  return allGlassFiberCategories.map((cat) => ({ category: cat.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!category) return {};
  return {
    title: `${category.name} | Glass Fiber Products`,
    description: category.description,
  };
}

export default async function GlassCategoryPage({ params }: Props) {
  const { category: slug } = await params;
  const category = allGlassFiberCategories.find((c) => c.slug === slug);
